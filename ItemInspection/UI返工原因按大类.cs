...
{
    public partial class UI返工原因按大类 : UserControl
    {
        ...
        {
            InitializeComponent();
            barEditItem3.EditValue = CPublic.Var.getDatetime().ToString("yyyy-MM-dd");
        ...
        private void gridView1_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            string sql = string.Format(@"select 生产工单号,返工数量,生产记录生产检验单主表.原规格型号,大类,生产记录生产检验单主表.负责人员,生产记录生产检验单主表.检验日期 
                                ...
                                and 基础数据物料信息表.大类='{2}' and 返工原因='{3}' order by 返工数量", Convert.ToDateTime(barEditItem1.EditValue), Convert.ToDateTime(barEditItem3.EditValue).AddDays(1)
                                 , dr["大类"], dr["返工原因"].ToString().Trim());
        ...
        private void fun_load()
        {
            ...
            string sql_1 = string.Format(@"select a.大类,返工原因,SUM(数量)as 数量  from 
                        (select 成品检验检验记录返工表.*,基础数据物料信息表.大类 ,生产记录生产检验单主表.原规格型号 from 成品检验检验记录返工表,生产记录生产检验单主表,基础数据物料信息表 
                        ...
                        group by a.大类,返工原因 order by 数量 ",
                   barEditItem1.EditValue, Convert.ToDateTime(barEditItem3.EditValue).AddDays(1).AddSeconds(-1));
