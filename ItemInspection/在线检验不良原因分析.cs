...
{
    public partial class 在线检验不良原因分析 : UserControl
    {
        ...
        private void 在线检验不良原因分析_Load(object sender, EventArgs e)
        {
        ...
        private void fun_search()
        {
            ...
            string sql = string.Format(@"select ltrim(rtrim(返工原因))返工原因,sum(数量)数量 from  [快速检验检验记录返工表] where 生产检验单号 
                                    in (select 生产检验单号 from 快速检验生产检验单主表 where  检验日期>='{0}' and 检验日期<='{1}' {2} ) 
        ...
        private void gridView1_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            string sql = string.Format(@"select 生产工单号,生产检验单号,返工数量,base.规格型号,负责人员,大类,convert(varchar(10),送检日期,120)生产日期 from  快速检验生产检验单主表 jyz,基础数据物料信息表 base 
                            where   jyz.物料编码= base.物料编码  and   检验日期>='{0}' and 检验日期<='{1}' {3}
                            and 生产检验单号 in (select 生产检验单号 from [快s速检验检验记录返工表] where 返工原因='{2}')  order by 返工数量 ", Convert.ToDateTime(barEditItem1.EditValue)
