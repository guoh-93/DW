...
namespace IAACA
{
    public partial class ui生产订单执行日报表 : UserControl
    {
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            DateTime t1 = new DateTime(Convert.ToInt32(comboBox1.Text.Trim()), Convert.ToInt32(comboBox2.Text.Trim()),1);
        ...
        {
            string s = string.Format(@"select  部门,物料编码,物料名称,规格型号,存货分类编码,存货分类,计量单位,日,sum(生产数量)生产数量 from (
select   case when 生产工单类型='小批试制' then '工艺部' else 车间名称 end as 部门,base.物料编码,base.物料名称,base.规格型号,存货分类编码,存货分类,base.计量单位
,DATEPART(dd,gd.生效日期)日,生产数量   from   生产记录生产工单表 gd 
left join 基础数据物料信息表 base on base.物料编码=gd.物料编码 where 生效日期 >'{0}' and 生效日期 <'{1}'  and (gd.关闭=0 or (gd.关闭=1 and 已检验数量>0)) )x
  group by 部门,物料编码,物料名称,规格型号,存货分类编码,存货分类,计量单位,日 ", t1, t2);
            ...
            s = string.Format(@"select  部门,物料编码,物料名称,规格型号,存货分类编码,存货分类,计量单位,日,sum(入库数量)入库数量 from (
 select  case when 生产工单类型='小批试制' then '工艺部' else 车间名称 end as 部门,base.物料编码,base.物料名称,base.规格型号,存货分类编码,存货分类,base.计量单位
,DATEPART(dd,rk.生效日期)日, 入库数量 from 生产记录成品入库单明细表  rk
 left join 生产记录生产工单表 gd on rk.生产工单号=gd.生产工单号 
 left join 基础数据物料信息表 base on base.物料编码=gd.物料编码
 where rk.生效日期 >'{0}' and rk.生效日期 <'{1}')x
  group by 部门,物料编码,物料名称,规格型号,存货分类编码,存货分类,计量单位,日", t1, t2);
            ...
            for (int i = 1; i <= t2.Day; i++)
            {
                dtM.Columns.Add(i.ToString() + "日生产数量",typeof(decimal));
                dtM.Columns.Add(i.ToString() + "日入库数量", typeof(decimal));
            }
            ...
            foreach (DataRow dr in dt_生产.Rows)
            {
                ...
                if (sr_生产.Length > 0)
                {
                    sr_生产[0][dr["日"].ToString() + "日生产数量"] = dr["生产数量"];
                ...
                else
                {
                    ...
                    r_生产["存货分类"] = dr["存货分类"];
                    r_生产["计量单位"] = dr["计量单位"];
                    r_生产[dr["日"].ToString() + "日生产数量"] = dr["生产数量"];
            ...
            foreach (DataRow dr in dt_入库.Rows)
            {
                ...
                if (sr_入库.Length > 0)
                {
                    sr_入库[0][dr["日"].ToString() + "日入库数量"] = dr["入库数量"];
                }
                else
                {
                    ...
                    r入库["存货分类"] = dr["存货分类"];
                    r入库["计量单位"] = dr["计量单位"];
                    r入库[dr["日"].ToString() + "日入库数量"] = dr["入库数量"];
                    dtM.Rows.Add(r入库);
                }
            }
            ...
            {
                if (!dc.ColumnName.Contains("日") && !dc.ColumnName.Contains("总"))
                {
                ...
                //if (dc.ColumnName.Contains("日")) 
                //{
                //    gb.Caption = dc.ColumnName.Substring(0,dc.ColumnName.IndexOf("日"));
            ...
            for (int i = 1; i <= t2.Day; i++)
            {
                ...
                bgc1.FieldName = i.ToString() + "日生产数量";
                bgc1.Caption = "生产数量";
                ...
                bgc2.FieldName = i.ToString() + "日入库数量";
                bgc2.Caption = "入库数量";
                bgc2.Visible = true;
        ...
        private void ui生产订单执行日报表_Load(object sender, EventArgs e)
        {
            ...
            comboBox2.Text = time.Month.ToString();
        }
