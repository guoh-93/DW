...
namespace BaseData
{
    public partial class frm员工劳保用品管理 : UserControl
    {
        #region  公有成员
        string strConn = "";
        string strConn_FS = "";
        ...
        private void frm员工信息维护_Load(object sender, EventArgs e)
        {
            try
            {
                da_日期_前.EditValue = Convert.ToDateTime(System.DateTime.Now.AddDays(-7).ToString("yyyy-MM-dd"));
                da_日期_后.EditValue = Convert.ToDateTime(System.DateTime.Now);
        ...
        private void fun_下拉框处理()
        {
            try
            {
                ...
                txt_工作鞋尺寸冬.Properties.Items.Clear();
                ...
                foreach (DataRow r in dt_属性.Rows)
                {
                    ...
                    if (r["属性类别"].ToString().Equals("工作鞋尺寸"))
                    {
                        ...
                        txt_工作鞋尺寸冬.Properties.Items.Add(r["属性值"].ToString());
                    }
                ...
                //部门表的下拉框
                dt_部门 = MasterSQL.Get_DataTable("select * from 人事基础部门表", CPublic.Var.strConn);
        ...
        private void fun_Check()
        {
            ...
            if (txt第一次鞋时间冬.Text == "")
            {
                txt第一次鞋时间冬.EditValue = DBNull.Value;
            }
            if (txt第二次鞋时间冬.Text == "")
            {
                txt第二次鞋时间冬.EditValue = DBNull.Value;
            }
            if (txt第一次长袖时间.Text == "")
            {
            ...
            //    cb_课室.EditValue = "";
            //}
            string sql = @"select *,case when([是否特岗]='true') then '是' else  '否' end as 特岗 from 人事基础员工表";
        ...
        {
            string sql = string.Format(@"select *,case when([是否特岗]='true') then '是' else  '否' end as 特岗 
        ...
        private void fun_刷新()
        {
          ...
          //  fun_主数据载入();
            ...
            {
                drM = dr[0];
                dataBindHelper1.DataFormDR(drM);   //写到界面
        ...
        //    //strguid = MasterFileService.BOLBUpload(System.IO.File.ReadAllBytes(pathName));  //上传文件
        ...
        private void fun_文件下载(string pathName, DataRow r)
        {
            ...
            //DataRow [] dl=dtP.Select(string.Format("员工号='{0}' and 文件名称='{1}'",strygh,r["文件名称"].ToString())); //找到需要下载文件的文件GUID
            //byte[] bs = MasterFileService.BOLBDownLoad(dl[0]["文件GUID"].ToString());
            //System.IO.File.WriteAllBytes(pathName, bs);    
        }
        ...
        //选择课室 给 课室编号 赋值
        private void cb_课室_EditValueChanged(object sender, EventArgs e)
        {
        ...
        private void sl_班组编号_EditValueChanged(object sender, EventArgs e)
        {
            ...
            //{
            //    txt_班组.Text = dt_班组.Select(string.Format("班组编号='{0}'", sl_班组编号.EditValue))[0]["班组"].ToString();
            //}
        }

        private void gvM_CustomDrawRowIndicator_1(object sender, DevExpress.XtraGrid.Views.Grid.RowIndicatorCustomDrawEventArgs e)
        {
            if (e.Info.IsRowIndicator && e.RowHandle >= 0)
            {
                e.Info.DisplayText = (e.RowHandle + 1).ToString();
