...
namespace ERPorg
{
    public partial class frm人事_仓库 : UserControl
    {
        #region 变量
        string strconn = CPublic.Var.strConn;
        DataTable dt;

        string sql = "select * from 人事记录组织仓库关系表";
        DataTable dt_下拉;
        DataTable dt_下拉仓库;
        DataTable dt_下拉仓库权限;
        #endregion
        ...
        private void frm人事_仓库_Load(object sender, EventArgs e)
        {
            try
            {
                fun_TLhead();
        ...
        private void fun_TLhead()
        {
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(sql, strconn))
            {
                ...
                foreach (DataRow r in dr)
                {
                    ...
                    head.SetValue("仓库号", r["仓库号"].ToString());
                    head.SetValue("仓库名称", r["仓库名称"].ToString());
                    head.SetValue("仓库权限", r["仓库权限"].ToString());
        ...
        void repositoryItemSearchLookUpEdit1_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            ...
            if (r_test.Length == 0)
            {
                ...
                treeList1.Selection[0].SetValue("工号", e.NewValue);
            }
            else
            {
        ...
        void repositoryItemSearchLookUpEdit3_EditValueChanging(object sender, DevExpress.XtraEditors.Controls.ChangingEventArgs e)
        {
            ...
            DataRow[] r = dt_下拉仓库.Select(string.Format("属性字段1='{0}'", e.NewValue));
            if (r.Length > 0)
            {
                dr["仓库名称"] = r[0]["属性值"];
                treeList1.Selection[0].SetValue("仓库名称", dr["仓库名称"].ToString());
                treeList1.Selection[0].SetValue("仓库号", e.NewValue);
            }
            else
            {
                treeList1.Selection[0].SetValue("仓库名称","");
                treeList1.Selection[0].SetValue("仓库号", "");
        ...
        {
            if (e.Column.Caption == "工号" || e.Column.Caption == "上级工号"||e.Column.Caption=="仓库号")
            {
                return;
        ...
        private void fun_TL(TreeListNode n)
        {
            try
            {
                ...
                foreach (DataRow r in dr)
                {
                    ...
                    nc.SetValue("仓库权限", r["仓库权限"].ToString());
        ...
        private void fun_添加下级()
        {
            ...
            if (n.GetValue("仓库号") != null)
            {
                nc.SetValue("仓库号", n.GetValue("仓库号").ToString());
                nc.SetValue("仓库名称", n.GetValue("仓库名称").ToString());
            }
            //**
        ...
        private void fun_保存界面数据()
        {
            ...
            if (treeList1.Selection[0].GetValue("仓库号") == null)
            {
                r["仓库号"] = "";
            }
            else
            {
                r["仓库号"] = treeList1.Selection[0].GetValue("仓库号");
            }
            if (treeList1.Selection[0].GetValue("仓库名称") == null)
            {
                r["仓库名称"] = "";
            }
            else
            {
                r["仓库名称"] = treeList1.Selection[0].GetValue("仓库名称");
            }
            r["仓库权限"] = treeList1.Selection[0].GetValue("仓库权限");
        }
        ...
        private void fun_下拉()
        {
            ...
            string sql_ck = "select 属性类别,属性值,属性字段1 from 基础数据基础属性表 where 属性类别='仓库类别' ";
            dt_下拉仓库 = new DataTable();
            using (SqlDataAdapter da = new SqlDataAdapter(sql_ck, strconn))
            {
                da.Fill(dt_下拉仓库);
            }

            string sql_ckqx = string.Format ("select 属性类别,属性值 from 基础数据基础属性表 where 属性类别='{0}'","仓库权限");
           // string sql_ckqx = string.Format("select * from 基础数据基础属性表 where 属性类别='{0}'", "仓库权限");
            dt_下拉仓库权限 = new DataTable();
            using (SqlDataAdapter  da=new SqlDataAdapter (sql_ckqx,strconn))
            {           
                 da.Fill(dt_下拉仓库权限);
            }
            ...
            repositoryItemSearchLookUpEdit3.DataSource = dt_下拉仓库;
            repositoryItemSearchLookUpEdit3.DisplayMember = "属性字段1";
            repositoryItemSearchLookUpEdit3.ValueMember = "属性字段1";

            repositoryItemSearchLookUpEdit4.DataSource = dt_下拉仓库权限;
