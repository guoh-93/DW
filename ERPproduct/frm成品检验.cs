...
{
    public partial class frm成品检验 : UserControl
    {
        //dataBindHelper1  用在保存处
        //dataBindHelper2  用在清空处
        //dataBindHelper3  用在盒贴处 
        //检验要求需要修改单元格格式，要能回车换行 
        //送检人员 就是 工单负责人 2016 03 11 记录
        string str_大类 = "";
        string cfgfilepath = "";
        ...
        public frm成品检验(string jianyandan)
        {
            str_检验单 = jianyandan;
            InitializeComponent();
            strcon = CPublic.Var.strConn;
        }
        public frm成品检验(string str生产工单, decimal str检验入库数量, string str入库单号, string str物料编码)
        {
            InitializeComponent();
            strcon = CPublic.Var.strConn;
            str_生产工单 = str生产工单;
...
#pragma warning disable IDE1006 // 命名样式
        private void frm成品检验_Load(object sender, EventArgs e)
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter("select 厂区 from 人事基础员工表 where 员工号 ='" + str + "'", CPublic.Var.strConn))
                {
                    ...
                    if (dt_厂区.Rows[0]["厂区"].ToString() == "一厂")
                    {
                    ...
                    if (CPublic.Var.localUserName == "admin")
                    {
                        panel12.Visible = true;
                        panel13.Visible = true;
                        this.gridColumn32.OptionsColumn.AllowEdit = true;
                        this.gridColumn32.Visible = true;
                    }
                    if (CPublic.Var.localUserName == "江升涛")
                    {
                        panel12.Visible = true;
                        panel13.Visible = true;
                        this.gridColumn32.OptionsColumn.AllowEdit = true;
                ...
                if (str_检验单 != "")
                {
                    string sql = string.Format("select * from 生产记录生产检验单主表 where 生产检验单号='{0}'", str_检验单);
                    ...
                    //dt_工单.Select(string.Format("生产工单号"));
                }
                //载入空的drr
                string sql_jyd = string.Format("select * from 生产记录生产检验单主表 where 1<>1 ");
                ...
                if (str_生产工单 != null)
                {
                    ...
                    if (ds.Length > 0)
                    {
                        ...
                        if (str_生产工单 != null) txt_checknum.Text = str_检验入库数量.ToString();
                    }
                }
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                //if (e.Column.ColumnName == "返工原因")
                //{
                //    DataRow[] ds = dt_返工原因表.Select(string.Format("fgyy = '{0}'", e.Row["返工原因"]));
                //    e.Row["返工编号"] = ds[0]["fgbh"];
                //}
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                ...
                {
                    sql = string.Format("select 员工号,姓名,部门,岗位 from 人事基础员工表 where 部门编号='{0}' and 在职状态='在职'", dt_车间信息.Rows[0]["生产车间"]);
                }
                else
                {
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                ...
                //                    {
                //                        sql = string.Format(@"select 生产记录生产工单表.*,人事基础部门表.部门名称,基础数据物料信息表.原ERP物料编号 from 生产记录生产工单表 
                //                                            left join 人事基础部门表 on 人事基础部门表.部门编号 = 生产记录生产工单表.生产车间 
                //                                            left join  基础数据物料信息表 on  基础数据物料信息表.物料编码 = 生产记录生产工单表.物料编码
                //                                            where 生产记录生产工单表.未检验数量 > 0 and 生产记录生产工单表.生效 = 1 and 生效日期 >= '2016-10-01 00:00:00' 
                //                            and 生产记录生产工单表.检验完成 = 0 and 生产记录生产工单表.关闭 = 0 and (生产记录生产工单表.完工 = 1 
                //                            or (生产记录生产工单表.部分完工=1 and   生产记录生产工单表.已检验数量<部分完工数 )) ");
                //                    }
                //                    else
                //                    {
                //                        sql = string.Format(@"select 生产记录生产工单表.*,人事基础部门表.部门名称,基础数据物料信息表.原ERP物料编号 from 生产记录生产工单表 
                //                            left join 人事基础部门表 on 人事基础部门表.部门编号 = 生产记录生产工单表.生产车间 
                //                            left join  基础数据物料信息表 on  基础数据物料信息表.物料编码 = 生产记录生产工单表.物料编码                         
                //                            where 生产记录生产工单表.未检验数量 > 0 and 生产记录生产工单表.生效 = 1 and 生效日期 >= '2017-01-01 00:00:00' 
                //                            and 生产记录生产工单表.检验完成 = 0 and 生产记录生产工单表.关闭 = 0 and 生产记录生产工单表.生产车间 = '{0}'
                //                            and (生产记录生产工单表.完工 = 1 or (生产记录生产工单表.部分完工=1 and 生产记录生产工单表.已检验数量<部分完工数 ) )", dt_生产.Rows[0]["生产车间"].ToString());
                //                    }
                string sql = @"select a.*,b.物料编码 from 生产记录生产工单表 a left join 基础数据物料信息表 b on a.物料编码 = b.物料编码
                where a.检验完成 = 0 and a.未检验数量 >0 and a.生效 = 1
                and (a.完工 = 1 or (a.部分完工=1 and a.已检验数量<部分完工数))"; //  and a.关闭 = 0 
                //                                  where  生产记录生产工单表.未检验数量 > 0 and 生产记录生产工单表.生效 = 1
                //                                  and 生产记录生产工单表.检验完成 = 0 and 生产记录生产工单表.关闭 = 0"
                //and (生产记录生产工单表.完工 = 1 or (生产记录生产工单表.部分完工=1 and 生产记录生产工单表.已检验数量<部分完工数 ))";
                using (SqlDataAdapter da = new SqlDataAdapter(sql, strcon))
                {
                    dt_工单 = new DataTable();
                    da.Fill(dt_工单);
        ...
        // 查看明细
        // Decimal dee = 0;
        //private void gv_scgd_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        //{
            ...
            try
            {
                ...
                if (dt_检验项目.Rows.Count == 0)
                {
                    ...
                    if (dt_检验项目.Rows.Count == 0)
                    {
                        ...
                        sql_检验项目 = string.Format("select * from ZZ_JYXM where dl = '{0}' order by xh", dt_物料信息.Rows[0]["存货分类编码"].ToString());
                        da_检验项目 = new SqlDataAdapter(sql_检验项目, strcon);
                        dt_检验项目 = new DataTable();
                        da_检验项目.Fill(dt_检验项目);
                        if (dt_检验项目.Rows.Count == 0)
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                string sql = "select * from 成品检验检验记录返工表 where 1<>1";
                dt_返工 = new DataTable();
                SqlDataAdapter da = new SqlDataAdapter(sql, strcon);
                da.Fill(dt_返工);
...
#pragma warning restore IDE1006 // 命名样式
        {
            ...
            string sql_Gd = "select * from 生产记录生产工单表 where 1<>1";
            ...
            using (SqlDataAdapter da = new SqlDataAdapter(sql_Gd, strcon))
            {
                ...
                using (SqlDataAdapter da1 = new SqlDataAdapter(sql_jyd, strcon))
                {
                    ...
                    drr["已检验数量"] = Convert.ToDecimal(txt_checknum.Text); // 20-4-3经查验 这边存的是 检验主表 这个是此次检验数量,
                    drr["未检验数量"] = Convert.ToDecimal(txt_productnum.Text) - Convert.ToDecimal(drr["已检验数量"]);//// 20-4-3经查验 这个字段没用 
                    ...
                    drr["生效日期"] = t;
                    decimal cc = Convert.ToDecimal(txt_总计合格率.Text.Trim('%')) / 100;
...
#pragma warning restore IDE1006 // 命名样式
        {
            ...
            string sql_检索 = string.Format(@"select a.*,b.物料编码 from 生产记录生产工单表 a left join 基础数据物料信息表 b on a.物料编码 = b.物料编码
                where a.检验完成 = 0 and a.关闭 = 0 and a.未检验数量 >0 and a.生效 = 1
              and (a.完工 = 1 or (a.部分完工=1 and a.已检验数量<部分完工数)) and a.生产工单号 = '{0}'", txt_shenchgdh.Text);

            DataTable dt_检索 = CZMaster.MasterSQL.Get_DataTable(sql_检索, strcon);


            if (dt_检索.Rows.Count > 0)
            {
                if (Convert.ToBoolean(dt_检索.Rows[0]["部分完工"]) == true)
                {

                    if (Convert.ToDecimal(txt_checknum.Text) > Convert.ToDecimal((Convert.ToDecimal(dt_检索.Rows[0]["部分完工数"]) - Convert.ToDecimal(dt_检索.Rows[0]["已检验数量"])).ToString()))
                    {
                        throw new Exception("填写的送检数量已超出");
                    }
                }
                else
                {
                    if (Convert.ToDecimal(txt_checknum.Text) > Convert.ToDecimal(dt_检索.Rows[0]["未检验数量"]))
                    {
                        throw new Exception("填写的送检数量已超出");
                    }
                }
            ...
            //if (txt_部门名称.Text == "")
            //{
            //    throw new Exception("请填写车间名称（一厂）或者（二厂）");
            //}
            //if (txt_部门名称.Text != "一厂" && txt_部门名称.Text != "二厂")
            //{
            //    throw new Exception("请正确填写车间名称（一厂）或者（二厂）");
            //}
            //19-12-5将完工限制添加到检验完成时
            decimal d_已检数量 = 0;
            string sql_gd = string.Format("select 生产工单号,已检验数量,生产数量 from  生产记录生产工单表 where 生产工单号 = '{0}'", txt_shenchgdh.Text);
            DataTable dt_gd = CZMaster.MasterSQL.Get_DataTable(sql_gd, strcon);
            DataRow[] dr_工单数 = dt_gd.Select(string.Format("生产工单号 = '{0}'",txt_shenchgdh.Text));
            d_已检数量 = Convert.ToDecimal(dr_工单数[0]["已检验数量"]) + Convert.ToDecimal(txt_checknum.Text);
            if(Convert.ToDecimal(dr_工单数[0]["生产数量"]) == d_已检数量)
            {
                string sql_待领 = string.Format("select * from  生产记录生产工单待领料明细表 where 生产工单号='{0}' and 完成 = 0   ", txt_shenchgdh.Text);
                DataTable dt111 = CZMaster.MasterSQL.Get_DataTable(sql_待领, strcon);
                if (dt111.Rows.Count>0)
                {
                    throw new Exception("该工单存在未领料，请联系相关人员确认");
                }
                else
                {
                    string sql_退料申请 = string.Format("select * from 工单退料申请表 where  生产工单号 ='{0}' and 完成 = 0 and 作废 =0", txt_shenchgdh.Text);
                    DataTable dt222 = CZMaster.MasterSQL.Get_DataTable(sql_退料申请, strcon);
                    if (dt222.Rows.Count > 0)
                    {
                        throw new Exception("该工单存在未完成退料申请单，请联系相关人员确认");
                    }
            ...
            {
                string sql_待领明细 = string.Format(@"select a.* from  生产记录生产工单待领料明细表 a 
                                      left join  生产记录生产工单待领料主表 b on a.生产工单号 = b.生产工单号
                                      where a.生产工单号='{0}' and 领料类型='工单领料'     and b.关闭 =0", txt_shenchgdh.Text);
                DataTable dt333 = CZMaster.MasterSQL.Get_DataTable(sql_待领明细, strcon);
                string sql_退料明细 = string.Format(@"select a.生产工单号,b.物料编码,sum(需退料数量)需退料数量,sum(已退料数量)已退料数量 from 工单退料申请明细表 b
                                                      left join 工单退料申请表 a  on a.待退料号 = b.待退料号 
                                                      where a.完成 = 0 and a.作废 = 0 and b.完成 = 0 and b.关闭 = 0 and b.作废 = 0  and 生产工单号 = '{0}'
                                                      group by a.生产工单号, b.物料编码  ", txt_shenchgdh.Text);
                DataTable dt444 = CZMaster.MasterSQL.Get_DataTable(sql_退料明细, strcon);

                decimal d_应领数 = 0;
                 
                foreach (DataRow dr_dlmx in dt333.Rows)
                {

                    d_应领数 = Convert.ToDecimal(dr_dlmx["BOM数量"]) * d_已检数量;
                    DataRow[] dr_退料明细 = dt444.Select(string.Format("物料编码 = '{0}'", dr_dlmx["物料编码"]));
                    if (Convert.ToDecimal(dr_dlmx["待领料总量"])>0)
                    {
                        if (dr_退料明细.Length > 0)
                        {

                            if (d_应领数 > (Convert.ToDecimal(dr_dlmx["已领数量"]) - (Convert.ToDecimal(dr_退料明细[0]["需退料数量"]) - Convert.ToDecimal(dr_退料明细[0]["已退料数量"]))))
                            {
                                throw new Exception("发料数量不足以检验完成这么多数量");
                            }
                        }
                        else
                        {
                            if (!Convert.ToBoolean(dr_dlmx["完成"]))
                            {
                                if (d_应领数 > (Convert.ToDecimal(dr_dlmx["已领数量"])))
                                {
                                    throw new Exception("发料数量不足以检验完成这么多数量");
                                }
                            }
            ...
            //20-5-12 检查 送检数量=合格数量+重检合格数+报废数量
            try
            {
                decimal dec_送检 = Convert.ToDecimal(txt_checknum.Text);
                decimal dec_合格 = Convert.ToDecimal(txt_passnum.Text);
                decimal dec_报废 = Convert.ToDecimal(txt_报废数.Text);
                decimal dec_重检合格 = Convert.ToDecimal(txt_重检合格数.Text);
                if(dec_送检!= dec_合格+ dec_报废+ dec_重检合格)
                {

                    throw new Exception("合格数量+报废数量+重建合格数不等于送检数量,请检查");
                }
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                ...
                //加载一遍
                string sql = string.Format("select * from 生产记录生产检验单主表 where 生产检验单号 = '{0}'", str_检验单号);
                dtM = new DataTable();
                SqlDataAdapter da = new SqlDataAdapter(sql, strcon);
                da.Fill(dtM);
                drr = dtM.Rows[0];
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                if (MessageBox.Show("产品名称：" + txt_chanpinName.Text + "\n" + "规格型号：" + txt_原规格型号.Text + "\n" + "是否生效？", "询问？", MessageBoxButtons.YesNo) == DialogResult.No)
                {
                    return;
                }
                ...
                gv_scgd.CloseEditor();
                gc_scgd.BindingContext[dt_工单].EndCurrentEdit();
                string sql_Gd = "select * from 生产记录生产工单表 where 1<>1";
                using (SqlDataAdapter da = new SqlDataAdapter(sql_Gd, strcon))
                {
                    DataRow[] dr = dt_工单.Select(string.Format("生产工单号='{0}'", txt_shenchgdh.Text));
                    ...
                    dr[0]["已检验数量"] = Convert.ToDecimal(dr[0]["已检验数量"]) + Convert.ToDecimal(txt_checknum.Text);
                    dr[0]["未检验数量"] = Convert.ToDecimal(dr[0]["未检验数量"]) - Convert.ToDecimal(txt_checknum.Text);
                    if (Convert.ToDecimal(dr[0]["未检验数量"]) <= 0)
                    {
                        dr[0]["检验完成"] = 1;
                        dr[0]["检验完成日期"] = time;
                        //此批整单报废 需要判断工单是否入库完成
                        //并且工单已经全部检验完成 
                        decimal c = Convert.ToDecimal(txt_总计合格率.Text.Trim('%')) / 100;
                        if (c == 0)
                        {
                            string s = string.Format("select count(*)cc from 生产记录生产检验单主表 where 完成=0 and 生产工单号='{0}'", txt_shenchgdh.Text);
                            DataTable t = CZMaster.MasterSQL.Get_DataTable(s, strcon);
                            if (Convert.ToDecimal(t.Rows[0]["cc"]) == 0) //所有检验单都已完成
                            {
                                dr[0]["完成"] = true;
                                dr[0]["完成日期"] = time;
                                //然后需要判断制令是否完成   这里  8月已经上线已生效制令直接转工单模块 制令与工单一对一 工单完成即制令完成 但是最好写完善一点
                                string zl = string.Format("select  * from 生产记录生产制令表 where 生产制令单号='{0}'", dr[0]["生产制令单号"]);
                                dt_zl = CZMaster.MasterSQL.Get_DataTable(zl, strcon);
                                if (Convert.ToDecimal(dt_zl.Rows[0]["未排单数量"]) == 0)
                                {
                                    zl = string.Format("select  count(*)cc from 生产记录生产工单表  where 生产制令单号='{0}' and 生产工单号<>'{1}' and 完成=0 and 关闭=0"
                                        , dr[0]["生产制令单号"], txt_shenchgdh.Text);
                                    t = CZMaster.MasterSQL.Get_DataTable(zl, strcon);
                                    if (Convert.ToDecimal(t.Rows[0]["cc"]) == 0) //所有工单都已完成
                                    {
                                        dt_zl.Rows[0]["完成"] = true;
                                        dt_zl.Rows[0]["完成日期"] = time;
                                    }
                        ...
                        //20160801,因为在检验单生效的时候已经计算过已检数量和未检数量，所以将stockcore中的代码注释
                        //8.1之前的数据，已检数量都是工单数的两倍
                    }
                    //11-12备注
                    //生效
                    //string sql = string.Format("select * from  生产记录生产检验单主表 where 生产检验单号='{0}'", txt_checkdanID.Text);
                    //DataTable dt = new DataTable();
                    //using (SqlDataAdapter da = new SqlDataAdapter(sql, strcon))
                    //{
                    //    da.Fill(dt);
                    //    dt.Rows[0]["生效"] = 1;
                    //    dt.Rows[0]["生效人员ID"] = CPublic.Var.LocalUserID;
                    //    dt.Rows[0]["生效人员"] = CPublic.Var.localUserName;
                    //    dt.Rows[0]["生效日期"] = System.DateTime.Now;
                    //  ;
                    //}

                    string sql_工单已检 = "select * from 生产记录生产工单表 where 1<>1";
                    string sql_主 = "select * from 生产记录生产检验单主表 where 1<>1";
                    string sql_检验明细 = "select * from 成品检验检验记录明细表 where 1<>1";
                    string sql_返工 = "select * from 成品检验检验记录返工表 where 1<>1";
                    string sql_序列号 = "select * from 生产检验单与产品序列号对应关系表 where 1<>1";
                    string sql_返工序列号 = "select * from 序列号返工原因对应表 where 1<>1";
                    SqlConnection conn = new SqlConnection(strcon);
                    conn.Open();
                    SqlTransaction ts = conn.BeginTransaction("成品检验");
                    try
                    {
                        ...
                        if (dt_zl != null && dt_zl.Columns.Count > 0)
                        {
                            sql_主 = "select * from 生产记录生产制令表 where 1<>1";
                            cmm_主 = new SqlCommand(sql_主, conn, ts);
                            da_主 = new SqlDataAdapter(cmm_主);
                            new SqlCommandBuilder(da_主);
                            da_主.Update(dt_zl);
                        }
...
#pragma warning restore IDE1006 // 命名样式
        {
            ...
            // else if (comboBox1.Text.Trim() == "芜湖德力西" || comboBox1.Text.Trim() == "芜湖德力西英文")
            //{
            //    label21.Text = "规格型号：";
            //    label22.Text = "物料名称：";
            //    label24.Text = "对方型号：";
            ...
            //else if (comboBox1.Text.Trim() == "宏美模板")
            //{
            //    label21.Text = "规格型号：";
            //    label22.Text = "物料名称：";
            //    label24.Text = "LOT/SN";
...
#pragma warning restore IDE1006 // 命名样式
        {
            ...
            string str = @" select b.*,a.*,e.* from mom_orderdetail b 
...
#pragma warning restore IDE1006 // 命名样式
        {
            string str = "select * from 生产记录生产工单表 where 备注5 ='" + str_生产工单 + "'";
            ...
            if (dt_判断工单是否存在.Rows.Count == 0 && dt_DW.Rows.Count > 0)
            {
                ...
                using (SqlDataAdapter da1 = new SqlDataAdapter("select * from 生产记录生产工单表 where 1<>1", CPublic.Var.strConn))
                {
                    da1.Fill(dt_生产工单);
...
#pragma warning restore IDE1006 // 命名样式
        {
            try
            {
                ...
                if (r["部分完工"].Equals(true))
                {
                    ...
                    txt_checknum.Text = (Convert.ToDecimal(r["部分完工数"]) - Convert.ToDecimal(r["已检验数量"])).ToString();

                }
                //    txt_checknum.Text = Convert.ToInt32(r["未检验数"].ToString()).ToString();


                txt_原规格型号.Text = r["规格型号"].ToString();
                txt_checknum.Text = Convert.ToDecimal(txt_checknum.Text).ToString("f2");
                if (r["生产工单号"].ToString() == str000)
                    txt_checknum.Text = str_检验入库数量.ToString("f2");
...
#pragma warning restore IDE1006 // 命名样式
        ...
        private void txt_报废数_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
        ...
        private void txt_checknum_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (e.KeyChar != 8 && !Char.IsDigit(e.KeyChar))
        ...
        private void txt_passnum_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (e.KeyChar != 8 && !Char.IsDigit(e.KeyChar))
