...
namespace PLCC
{
    class PLCActResult
    {
        ...
        /// 延迟多少时间后开始统计.
        /// </summary>
        public int iTimeDelay = 500;


        public int iTimeDleay_1 = 100;
        #endregion
        ...
        #region 类相关
        public PLCActResult(MachineAdapter mad)
        {
            ///以下这三类数据可以直接去数据库获取.  
            try
            {
        ...
        private void fun_动作检查_机构检查和触点电压检查()
        {
            ...
            if (drP["动作ID"].ToString() == "40")
            {
                if (mad.T_子动作时间.TotalMilliseconds >= iTimes)
                {
                    System.Threading.Thread.Sleep(iTimeDleay_1);
                    ///AQUA
                    ///超时出错处理
            ...
            if (drP["动作ID"].ToString() == "41")
            {
                ...
                else
                {
                    if (bl激光分闸 == true)
                    {
                        ...
                        int iTime_SX = int.Parse(drP["P2"].ToString());
                        int iTime_XX = 0;
                        try { iTime_XX = int.Parse(drP["P3"].ToString()); }catch { };
