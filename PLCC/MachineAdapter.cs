...
namespace PLCC
{
    ...
    public class MachineAdapter
    {
        ...
        public DateTime T_工作时间 = System.DateTime.Now;
       ...
       // public Boolean blReadyWrok = false;
        ...
        //V12
        public Boolean V12电压状态 = false;
        public Decimal V12_跳跃电压 = 0;
        public Decimal V12_开始电压 = 0;
        public Decimal V12_目标电压 = 0;
        public int V12_跳动时间 = 0;
        public Boolean blV12add = true;
        ...
        public void setGoNext()
        {
            //if (iDZJP == 0 || iDZJP == 2)
            {
                try
                {
                    ...
                    //if(PLC_CURR_POS == dt_检测组动作子表.Rows.Count)
                    //{
                    //    PLC_CURR_POS = -1;
                    //}
                }
        ...
        {
            int Time_参数 = 0;
            T_工作时间 = System.DateTime.Now;
            if (dr_Curr["动作ID"].ToString() == "12")
            {
                r12 = dr_Curr;
                ...
                V12_开始电压 = Convert.ToDecimal(dr_Curr["P2"].ToString());
                V12_目标电压 = Convert.ToDecimal(dr_Curr["P4"].ToString());
                V12_跳跃电压 = Convert.ToDecimal(dr_Curr["P5"].ToString());
                V12_跳动时间 = int.Parse(dr_Curr["P1"].ToString()) * 1000;
        ...
        void tm指令分析_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                ...
                #region V12 +
                //if (V12电压状态 == false) return;
                if (V12电压状态 == true && blwork == true)
                {
                    V12_itime计时 = (int)(System.DateTime.Now - Vtime).TotalMilliseconds;
                    if (blV12add == true)
                    {
                        //电压正向跳动
                        if (V12_itime计时 >= V12_跳动时间 && System.Decimal.Parse(V额定电压) <= V12_目标电压)
                        {
                            //改变电压，当前电压 + V12_跳跃电压
                            V额定电压 = (Convert.ToDecimal(V额定电压) + V12_跳跃电压).ToString();
                            //try
                            //{
                            //    vSyn++;
                            //    if (vSyn >= 999)
                            ...
                            //}
                            //catch { }
                            VCControlAdapter.fun_设置可编程电源电压(Convert.ToDecimal(230), (System.Decimal)50);
                            System.Threading.Thread.Sleep(2000);
                            VCControlAdapter.fun_设置可编程电源电压(Convert.ToDecimal(V额定电压), (System.Decimal)50);
                            CZMaster.MasterLog.WriteLog(string.Format("*aqua* {0}: POS:{1} 连续电压 ：{2}", strMachineName, dr_Curr["动作POS"], V额定电压));
                            Vtime = System.DateTime.Now;
                            V12_itime计时 = 0;
                        }

                        if (System.Decimal.Parse(V额定电压) > V12_目标电压)
                        {
                            V12电压状态 = false;
                        }
                    }
        ...
        private void fun_JSGZSJ()
        {
            ...
            try
            {
                ...
                //计算工作时间
                //if (rr["动作ID"].ToString() == "00")
                //{
                //    Is开始计时_总 = true;
                //}
                //if (Is开始计时_总 == true && rr["动作ID"].ToString() != "FE")
                //{
                //    T_工作时间 += tm指令分析.Interval;
                //}

                //if (rr["动作ID"].ToString() == "FE" || PlcAR.iPLCActResult == 2 || PLC_S_Work == "false")
                //{
                //    Is开始计时_总 = false;
                //}
        ...
        private void fun_PLC指令的后续处理()
        {
            try
            {
                if (plc.PLC_S != "")   //S指令不为空
                {
                    ...
                    if (PLC_S_Ready == "1" && PLC_S_PLC_X.Substring(12,1) == "1")
                    {
                        if (strCPSN != "")
                        {
    ...
    public class ResultR
    {
        ...
        public string PLC_R_动作ID = "";
        public string PLC_R_动作POS = "";
        public string PLC_R_结果判定 = "";   //1: PASS 2:NG
        public string PLC_R_错误代码 = "";
        public string PLC_R_错误描述 = "";
        public string PLC_R_R1 = "";
        public string PLC_R_R2 = "";
        public string PLC_R_R3 = "";
        public string PLC_R_R4 = "";
