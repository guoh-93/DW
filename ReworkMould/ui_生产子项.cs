...
{
    public partial class ui_生产子项 : UserControl
    {
        ...
        public ui_生产子项(DataTable dt_total, string str)
        {
            InitializeComponent();
            dt_生产子项 = dt_total;
            str_产品编码 = str;
        ...
        private void ui_生产子项_Load(object sender, EventArgs e)
        {
            try
            {
 ...
 (select  产品编码,子项编码,仓库号,仓库名称,WIPType,子项类型,数量,bom类型,1 as level from 基础数据物料BOM表 
  ...
  left join 基础数据物料信息表  fx  on fx.物料编码=a.产品编码 where base.自制 = 1
  group by 子项编码", str_产品编码);
            ...
            DataView dv = new DataView(dt_生产子项);
